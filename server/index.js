...
app.post("/addOweFavour", async (req, res) => {
  try {
    ...
    const checkUser = await pool.query(
      "SELECT user_name,user_id from userdata where user_name=$1",
      [recievinguser]
    );
    ...
    if (checkUser.rows.length > 0) {
      const newOweFavour = await pool.query(
        "INSERT INTO owefavour(user_name,title,favour_description,rewards,recieving_username,favour_image)VALUES($1,$2,$3,$4,$5,$6) RETURNING * ",
